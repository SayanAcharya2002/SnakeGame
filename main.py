...
if __name__=="__main__":
  ...
  best_model_path=os.path.join(MODEL_DIR,f"QLearningAgentBest_{WIDTH//PLAYER_SIZE}x{HEIGHT//PLAYER_SIZE}.pkl")
  latest_model_path=os.path.join(MODEL_DIR,f"QLearningAgentLatest_{WIDTH//PLAYER_SIZE}x{HEIGHT//PLAYER_SIZE}.pkl")
  ...
  agent=QLearning.agent(
    num_states_limit=(WIDTH//PLAYER_SIZE,)*2+(2*WIDTH//PLAYER_SIZE+1,)*2,
  ...
  for episode in range(num_episode):
    ...
    while run:
      ...
      if agent_playing:
        ...
        # action=agent.do_action(cur_state,eval_mode)
        # if action==4:
        #   pass # do nothing
        # else:
        #   PLAYER.change_dir(ACTION_ORDERING[action])
        cur_state=get_state_encoding(PLAYER,apples)
        action=agent.do_action(cur_state,eval_mode)
        if action==2:
          pass # do nothing
    ...
    if not is_render:
      if episode%episode_factor==0:
        ...
        # pos=sum(i  for i in agg_reward_list[-1:-(episode_factor+1):-1] if i>0)
        # neg=sum(i  for i in agg_reward_list[-1:-(episode_factor+1):-1] if i<0)
